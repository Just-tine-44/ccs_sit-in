...
// Query to get top 5 students based on lab sessions
$sessionsQuery = "SELECT u.id, CONCAT(u.firstname, ' ', u.lastname) as name, u.email, u.level, u.course, u.profileImg, 
         COUNT(s.sit_in_id) as session_count,
         SUM(TIMESTAMPDIFF(MINUTE, s.check_in_time, s.check_out_time)) as total_minutes
         FROM users u
         LEFT JOIN curr_sit_in s ON u.id = s.user_id AND s.status = 'completed'
         GROUP BY u.id
...
// Query to get top 5 students based on earned points only
$pointsQuery = "SELECT u.id, CONCAT(u.firstname, ' ', u.lastname) as name, u.email, u.level, u.course, u.profileImg, 
         COALESCE((SELECT SUM(points_earned) FROM student_points WHERE user_id = u.id), 0) as total_points
         FROM users u
         LEFT JOIN student_points sp ON u.id = sp.user_id
         GROUP BY u.id
...
$pointsStatsQuery = "SELECT SUM(points_earned) as total_points_awarded FROM student_points";
